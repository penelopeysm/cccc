...
(* For right shift, we need SAR for signed operands and SHR for unsigned. Right now we
   only have signed operands (i.e. int literals and combinations thereof) so we only have
   SAR. For left shift SAL and SHL are the same.
...
type instruction =
  ...
  | AllocateStack of { size : Natural.t }
[@@deriving sexp_of]
...
module Emit : sig
  ...
  let emit_register ?(sz : size = DWord) (buf : Buffer.t) (reg : register) : unit =
    let reg_name =
      match sz, reg with
      | QWord, AX -> "%rax"
      | DWord, AX -> "%eax"
  ...
  let emit_operand ?(register_size : size = DWord) (buf : Buffer.t) (op : operand) : unit =
    match op with
    ...
    | Stack stackpos ->
      Buffer.add_string buf (stackpos |> Natural.negate |> string_of_int);
  ...
  let emit_instruction (buf : Buffer.t) (inst : instruction) : unit =
    ...
    | AllocateStack { size } ->
      ...
      Buffer.add_string buf (size |> Natural.to_int |> string_of_int);
  ...
  let emit_func (buf : Buffer.t) (func : func) : unit =
    match func with
    | Function { name; insts } ->
      (* On macOS, x86 symbol names are prefixed with underscores -- not necessary on
         Linux *)
      let mangled_name =
        match Os.current_os with
        | MacOS -> "_" ^ name
        | Linux | Unknown -> name
      in
      ...
      List.iter ~f:(fun inst -> emit_instruction buf inst) insts
  ;;
  ...
  let string_of_asm = function
    | Programme { entry } ->
      ...
      (match Os.current_os with
       | Linux -> Buffer.add_string buf "   .section .note.GNU-stack,\"\",@progbits\n"
       | MacOS | Unknown -> ());
      Buffer.contents buf
  ;;
end

(* The ASM type above is more lax than it should be because it can represent instructions
   that are invalid. The most obvious case is a pseudoregister that has not actually been
   allocated to a real register or stack slot. However there are also some other
   constraints, for example the fact that the destination of an `addl` instruction must
   not be an immediate value. It is the job of the fixup phase to ensure that these
   constraints are satisfied. This module helps that by providing a tool to check that
   this is indeed the case to avoid emitting malformed assembly code.
*)
module Validate : sig
  val validate : t -> bool
end = struct
  (* This basically means not a pseudoregister. *)
  let is_reg_mem_or_imm = function
    | PseudoRegister _ -> false
    | _ -> true
  ;;

  let is_reg_or_mem = function
    | Register _ | Stack _ -> true
    | _ -> false
  ;;
  ...
  let validate_binary op left dst =
    ...
    | Sal | Sar -> equal_operand left (Register CX) || is_imm left
    (* imul destination must be a register *)
    | Imul -> is_reg dst
  ;;

  let validate_instruction inst =
    match inst with
    | Movl { src; dst } ->
      is_reg_mem_or_imm src
      && is_reg_or_mem dst
    ...
    | AllocateStack { size } -> true
  ;;

  let validate_instruction_with_reporting (fn_name : string) (inst : instruction) : bool =
    let result = validate_instruction inst in
    if not result
    then
      eprint_s
        [%message
          "Invalid instruction in function" (fn_name : string) (inst : instruction)];
    result
  ;;
  ...
  let validate (Programme { entry }) =
    ...
    | Function { name; insts } ->
      let results =
        List.map ~f:(fun inst -> validate_instruction_with_reporting name inst) insts
      in
      List.for_all ~f:Fun.id results
  ;;
end
