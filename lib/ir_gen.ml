...
(* IR generation from AST. Because the IR is linear, we need to essentially continually
   append instructions to the end of the list. We achieve this using a builder module that
   remembers its progress so far. *)
...
(* The general pattern for these lowering functions is to add instructions to the builder
   as we go, and return the Ir.Var that holds the result of the expression (because it
   might be used in a later instruction). This works nicely for expressions; we haven't
   yet gotten to statements! *)
...
let rec lower_exp (exp : Ast.exp) (b : Builder.t) : Ir.value =
  ...
  | Ast.UnaryOp { op; operand } ->
    ...
    in
    let new_op = Ir.UnaryOp { op = ir_op; src = operand_value; dst = dst_var } in
...
let lower_func = function
  | Ast.Function { name; body } ->
    Ir.Func { name = Ast.get_identifier_name name; insts = lower_statement body }
;;
