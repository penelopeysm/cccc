...
let try_compilation_stage (command: string) (stage_name: string) (cleanup: string list) : unit =
...
let main ~(fname: string) ~(retain_assembly: bool) ~(run: bool) : unit =
  ...
  try_compilation_stage link_command "Assembling and linking" [preproc_fname; assembly_fname; executable_fname];
  ...
  if run then
    exit (Sys.command executable_fname)
  else
    exit 0
;;
