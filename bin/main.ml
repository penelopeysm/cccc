...
let print_with_box (styles : ANSITerminal.style list) (title : string) (content : string)
  : unit
  =
  let styles_with_bold =
    if List.mem ANSITerminal.Bold styles then styles else ANSITerminal.Bold :: styles
  in
  let content =
    let n = String.length content in
    if n > 0 && content.[n - 1] = '\n' then String.sub content 0 (n - 1) else content
  in
  let content_lines = String.split_on_char '\n' content in
  let inner_width =
    List.fold_left max (String.length title) (List.map String.length content_lines)
  in
  let hbar =
...
let main
  ...
  In_channel.with_open_text input_fname (fun inx ->
    ...
    if not (Asm.Validate.validate asm)
    then (
      print_with_box
  ...
  let command =
    ...
    and+ output_fname =
      named_opt
        [ "o" ]
        string
        ~doc:"Path to the output file. If not provided, output will be printed to stdout"
    and+ dump_ast = flag [ "a"; "dump-ast" ] ~doc:"Additionally dump the AST to stderr"
    and+ dump_ir = flag [ "i"; "dump-ir" ] ~doc:"Additionally dump the IR to stderr"
    and+ parse =
      flag
        [ "parse" ]
